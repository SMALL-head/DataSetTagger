...
public class PublisherSecurityConfiguration {
    ...
    @Bean
    public SecurityFilterChain filterChain2(HttpSecurity httpSecurity, UserDetailsService userDetailsService, AccessDeniedHandler accessDeniedHandler) throws Exception {
...
//                    requestMatchers("/api/tagger/**").hasRole("TAGGER").
                    ...
                    requestMatchers("/publisher/login").permitAll().
                    anyRequest().authenticated(). // 所有其他请求一律需要认证
                    and().
                    formLogin(). //.loginPage todo: 前端登陆界面完成后指向它
                    loginProcessingUrl("/api/publisher/login"). // 前端action中的url，匹配后将会将登录认证信息传给bean认证
